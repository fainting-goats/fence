...
if __name__ == '__main__':
    ...
    import numpy
    edgesX = range(0,60,5)+range(60,100,10)+range(100,200,20)+range(200,320,40)+range(320,800,80)
    binsX = numpy.array(edgesX,dtype='d')
    edgesY = range(0,180,45)+[180]
    binsY = numpy.array(edgesY,dtype='d')
    h = ROOT.TH2F('ptll','ptll;ptll;dphillmet',len(binsX)-1,binsX,len(binsY)-1,binsY)
    h.SetMarkerStyle(24)
...
#     c.SetLogy()
    basic = 'trigger==1 && nextra==0 && !zveto && njet==1'
    ...
    hlo.SetTitle(cutlo)
    hhi.SetTitle(cuthi)
    print 'lomet:',dyll.Draw('dphillmet*180/pi:ptll >> ptll_lo',cutlo,'goff')

    print 'himet:',dyll.Draw('dphillmet*180/pi:ptll >> ptll_hi',cuthi,'goff')


#     hs = ROOT.THStack('ptll_hilomet',';ptll;dphillmet')
#     hs.Add(hlo)
#     hs.Add(hhi)
